...
import * as users from './usersModel.mjs';
...
app.get ('/create', asyncHandler(async (req,res) => { 
    const user = await users.createUser(
        ...
        req.query.age, 
        req.query.email,
        req.query.phoneNumber   // added this
        )
    res.send(user);
}));
...
function userFilter(req) {
    ...
    if (req.query._id !== undefined) {
        ...
        filter.phoneNumber = req.query.phoneNumber ;  // added this
    } 
    return filter;
}
...
app.get('/delete', (req, res) => {
    if (req.query._id !== undefined) {
        ...
        deleteByProperty(req, res);     // calls the function above and tracks deleted count
    }
});
...
app.get('/update', (req, res) => {
    ...
    users.findById(req.query._id)
        ...
        .catch(error => {
            console.error(error);
            res.json({ Error: 'really bad error' });
        });
